const STORAGE_KEY = "ai-content-recommender-v5-polished-ui";
...
function renderInterestChips() {
  ...
  const keywords = getProfile().positiveKeywords.slice(0, 10);
  if (!keywords.length) {
    container.innerHTML = `<span class="empty-chip">아직 긍정 반응이 없습니다. 좋아요·북마크·공유를 눌러보세요.</span>`;
    return;
  }

  container.innerHTML = keywords
    .map((item) => `<span class="chip">#${escapeHtml(item.keyword)} <small>${item.weight}</small></span>`)
    .join("");
}
...
function renderContents() {
  ...
  if (!state.items.length) {
    grid.innerHTML = `<div class="empty">아직 추천 결과가 없습니다.<br />위 검색창에 관심 키워드를 입력하면 실제 콘텐츠를 찾아 보여줍니다.</div>`;
    return;
  }
...
function createCard(item) {
  ...
  const imageUrl = item.imageUrl || item.thumbnail || item.thumbnailUrl || "";
  const mediaHtml = imageUrl
    ? `<img class="card-image" src="${escapeAttribute(imageUrl)}" alt="${escapeAttribute(item.title)} 썸네일" loading="lazy" onerror="this.parentElement.classList.add('no-image'); this.remove();" />`
    : `<div class="image-placeholder">${escapeHtml((item.type || item.category || "콘텐츠").slice(0, 2))}</div>`;

  card.innerHTML = `
    <div class="card-media">
      ${mediaHtml}
      <span class="card-type">${escapeHtml(item.type || item.category || "콘텐츠")}</span>
      <span class="score">${score}</span>
    </div>
    <div class="card-body">
      <h3>${escapeHtml(item.title)}</h3>
      <p>${escapeHtml(item.description || "키워드와 관련된 추천 콘텐츠입니다.")}</p>
      <div class="reason">${escapeHtml(item.reason || "입력한 키워드와 연관성이 높습니다.")}</div>
      <div class="tags">${tags.map((tag) => `<span class="tag">#${escapeHtml(tag)}</span>`).join("")}</div>
      ${url ? `<a class="source-link" href="${escapeAttribute(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(sourceName)}에서 보기</a>` : `<span class="source-link" aria-disabled="true">데모 추천</span>`}
    </div>
    <div class="actions">
      <button data-action="liked" title="좋아요">좋아요</button>
      <button data-action="bookmarked" title="북마크">북마크</button>
      <button data-action="shared" title="공유">공유</button>
      <button data-action="disliked" title="관심없음">관심없음</button>
    </div>
  `;
...
function schedulePersonalRecommendations() {
  ...
  const topKeywords = getProfile().positiveKeywords.map((x) => x.keyword).slice(0, 5).join(", ");
  $("personalText").textContent = topKeywords
    ? `분석된 관심사: ${topKeywords}. 이 키워드들을 융합해 새로운 콘텐츠를 추천합니다.`
    : "좋아요, 북마크, 공유한 키워드를 종합해 새로운 실제 콘텐츠를 추천합니다.";

  clearTimeout(state.personalTimer);
  state.personalTimer = setTimeout(loadPersonalRecommendations, 600);
}
